...
/// [`FromQueryResult`].
///
/// The `id` field uses a backend-specific fallback: try native `Uuid`
/// first (Postgres, MySQL with `binary(16)` + uuid feature), then fall
/// back to `String` + parse (SQLite stores UUIDs as hex `TEXT`).
///
...
// Manual FromQueryResult: try native Uuid first (Postgres, MySQL with
// binary(16) + uuid feature), fall back to String + parse (SQLite stores
// UUIDs as hex TEXT).
impl FromQueryResult for JobRow {
    fn from_query_result(row: &QueryResult, prefix: &str) -> Result<Self, DbErr> {
        let id = match row.try_get::<Uuid>(prefix, "id") {
            Ok(id) => id,
            Err(_) => {
                let s: String = row.try_get(prefix, "id")?;
        ...
        Ok(Self {
            id,
            queue: row.try_get(prefix, "queue")?,
            job_type: row.try_get(prefix, "job_type")?,
            payload: row.try_get(prefix, "payload")?,
            status: row.try_get(prefix, "status")?,
            attempts: row.try_get(prefix, "attempts")?,
            max_retries: row.try_get(prefix, "max_retries")?,
            run_at: row.try_get(prefix, "run_at")?,
            started_at: row.try_get(prefix, "started_at")?,
            locked_until: row.try_get(prefix, "locked_until")?,
            finished_at: row.try_get(prefix, "finished_at")?,
            last_error: row.try_get(prefix, "last_error")?,
            trace_id: row.try_get(prefix, "trace_id")?,
            created_at: row.try_get(prefix, "created_at")?,
        })
    }
}
