        ...
        Some(Commands::Jobs { command }) => match command {
            ...
            JobsCommands::List { failed } => {
                ...
                #[cfg(not(feature = "jobs"))]
                {
                    let _ = failed;
                    eprintln!(
                        "{} The jobs list command requires the jobs feature and a database driver. \
                             For PostgreSQL, reinstall with: cargo install rapina-cli --features jobs,sea-orm/sqlx-postgres",
                        "Error:".red().bold()
                    );
                    std::process::exit(1);
                }
